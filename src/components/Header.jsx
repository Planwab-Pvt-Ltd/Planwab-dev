...
const CategoryButton = ({ category, imageSrc, active, onClick, buttonRef }) => (
    <button
        ...
        className={`
            relative flex items-center justify-center space-x-2.5 px-4 py-2.5 mx-0.5 rounded-lg
...
const PlannerDropdown = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 py-6 px-6 z-50 transform transition-all duration-300 ease-out animate-in fade-in-0 slide-in-from-top-2">
...
const LocationDropdown = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 py-4 z-50 transform transition-all duration-300 ease-out animate-in fade-in-0 slide-in-from-top-2">
            ...
            <div className="max-h-48 overflow-y-auto">
                {filteredCities.map((city, index) => (
                    ...
                    >
                        ...
                        <span className={`${
                            currentCity === city ? 'text-blue-700 font-medium' : 'text-gray-700'
                        }`}>
                            {city}
                        </span>
                    </button>
...
const ProfileDropdown = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-200 py-2 z-50 transform transition-all duration-300 ease-out animate-in fade-in-0 slide-in-from-top-2">
...
const Header = () => {
  ...
  return (
    <header className={`
        fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-lg z-50 
        transition-all duration-500 ease-out
        ${isScrolled ? 'border-b border-gray-200 shadow-xl' : 'border-b border-gray-100 shadow-sm'}
    `}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="flex justify-between items-center h-20">
              ...
              <div ref={tabsContainerRef} className="relative flex-1 flex justify-center items-center h-full max-w-lg">
                  <div className="flex items-center bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-1.5 shadow-inner border border-gray-200/50">
                      {categories.map((cat) => (
                          <CategoryButton
                              key={cat.name}
              ...
              <div className="flex items-center space-x-2">
                <div className="relative" ref={plannerRef}>
                    <button 
                        ...
                        className="hidden lg:flex items-center space-x-1 font-semibold text-gray-700 hover:text-gray-900 hover:bg-gradient-to-r hover:from-gray-100 hover:to-gray-50 rounded-lg px-4 py-2.5 transition-all duration-300 ease-out hover:scale-105 hover:shadow-lg border border-transparent hover:border-gray-200"
                    >
                ...
                <div className="relative" ref={locationRef}>
                    <button 
                        ...
                        className="p-2.5 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all duration-300 ease-out hover:scale-110 hover:shadow-lg hover:rotate-12"
                    >
                        <Globe size={20} />
                    </button>
                    <LocationDropdown 
                ...
                <div className="relative" ref={profileRef}>
                    <button 
                        ...
                        className="flex items-center space-x-2.5 border border-gray-300 rounded-xl p-1.5 pl-3 pr-2 shadow-lg hover:shadow-xl transition-all duration-400 ease-out hover:scale-105 bg-white hover:bg-gradient-to-r hover:from-white hover:to-gray-50"
                    >
                        <Menu size={16} className="text-gray-700" />
                        <div className="w-8 h-8 bg-gradient-to-br from-gray-700 to-gray-900 rounded-lg flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300">
