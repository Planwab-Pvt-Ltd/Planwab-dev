    ...
    <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 py-2 !z-50 transform transition-all duration-300 ease-out animate-in fade-in-0 slide-in-from-top-2 px-2">
      ...
      <SignedOut>
        <div className="flex flex-col gap-2 border-t border-gray-100 dark:border-gray-700 pt-2 justify-center items-center">
          <SignInButton>
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="w-[90%] px-4 py-3 flex items-center space-x-3 rounded-xl bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-800/50 transition-colors duration-200 group cursor-pointer justify-center">
          ...
          <SignUpButton>
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="w-[90%] px-4 py-3 flex items-center space-x-3 rounded-xl bg-green-100 dark:bg-green-700/30 hover:bg-green-200 dark:hover:bg-green-800/50 transition-colors duration-200 group cursor-pointer justify-center">
      ...
      <SignedIn>
        <div className="border-t border-gray-100 dark:border-gray-700 pt-2">
          <button className="w-full px-4 py-3 text-left hover:bg-red-50 dark:hover:bg-red-900/40 transition-all duration-200 flex items-center space-x-3 group rounded-lg" onClick={() => signOut()}>
    ...
    <div className="lg:hidden fixed inset-0 z-[9999]">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-4 space-y-1">
            <div className="mb-3">
              <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Event Categories</h3>
            </div>
            {categories.map((cat) => {
              ...
              return (
                <Link href={categoryPath} key={cat.name} onClick={onClose}>
                  <div className={`flex items-center space-x-4 p-3 rounded-xl text-md font-semibold transition-all duration-200 ${
                    isActive 
              ...
              {openAccordion === "planner" && (
                ...
                >
                  <div className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-4 text-center mt-2 border border-blue-100 dark:border-gray-600">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">Join our community and turn your passion into a profession.</p>
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-1">
                      {filteredCities.slice(0, 10).map((city, index) => (
                        ...
                        >
                          ...
                          <span className={`text-sm ${currentCity === city ? "text-white font-medium" : "text-gray-700 dark:text-gray-300"}`}>
                            {city}
                          </span>
                        </button>
                      ))}
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-4 mt-2 border border-purple-100 dark:border-gray-600">
                    <SignedIn>
                      <div className="flex items-center space-x-3 mb-4 p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-600">
                        <img src={user?.imageUrl} alt={user?.fullName || "User"} className="w-12 h-12 rounded-full border-2 border-blue-200 dark:border-blue-400" />
                        <div>
                    ...
                    <SignedOut>
                      <div className="text-center space-y-4">
                        ...
                        <div>
                          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-1">Welcome to planWAB</h3>
                          <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">Join us to plan amazing events</p>
                        </div>
                        <div className="space-y-3">
                          <SignInButton>
...
export default function Header() {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 !z-[9000] transition-all duration-500 ease-out rounded-b-3xl ${isScrolled ? "bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg border-b border-gray-200 dark:border-gray-800 shadow-xl" : "bg-white/10 dark:bg-black/10 backdrop-blur-2xl"}`}>
          ...
          <div className="flex justify-between items-center h-20">
            <div className="flex-shrink-0">
              <Link href="/" className="flex items-center space-x-2 cursor-pointer group transition-all duration-400 ease-out hover:scale-110" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
                ...
                <span className={`text-2xl font-bold tracking-tight transition-all duration-400 ease-out bg-gradient-to-r from-slate-800 to-slate-600 dark:from-slate-200 dark:to-slate-400 bg-clip-text text-[#f59e0b] dark:text-transparent ${isHovered ? "scale-105" : ""}`}>planWAB</span>
              </Link>
            </div>

            <div className="hidden lg:flex flex-1 justify-center items-center h-full max-w-lg">
              <div className={`flex items-center bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800/50 dark:to-gray-900/50 rounded-2xl p-1.5 shadow-inner border border-gray-200/50 dark:border-gray-700/50`}>
                {categories.map((cat) => {
                  const categoryPath = `/events/${cat.name.toLowerCase()}`;
                  const isActive = pathname === categoryPath || pathname === `/plan-my-event/${cat.name.toLowerCase()}`;
                  return (<Link href={categoryPath} key={cat.name} passHref><CategoryButton category={cat.name} imageSrc={cat.image} active={isActive} /></Link>);
                })}
              </div>
            </div>

            <div className="hidden lg:flex items-center space-x-2">
              <button onClick={toggleTheme} className="p-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-all duration-300 ease-out hover:scale-110 hover:shadow-lg cursor-pointer">{theme === "dark" ? <Sun size={20} /> : <Moon size={20} />}</button>
              ...
              <div className="relative" ref={locationRef}>
                <button onClick={() => handleDropdownToggle("location")} className="p-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-all duration-300 ease-out hover:scale-110 hover:shadow-lg hover:rotate-12 cursor-pointer"><Globe size={20} /></button>
                <LocationDropdown isOpen={openDropdown === "location"} onClose={() => setOpenDropdown(null)} />
              </div>
              <div className="relative" ref={profileRef}>
                <button onClick={() => handleDropdownToggle("profile")} className="flex items-center space-x-2.5 border border-gray-300 dark:border-gray-700 rounded-2xl p-1.5 pl-3 pr-2 shadow-lg hover:shadow-xl transition-all duration-400 ease-out hover:scale-105 bg-white dark:bg-gray-800 hover:bg-gradient-to-r hover:from-white hover:to-gray-50 dark:hover:from-gray-700 dark:hover:to-gray-600">
                  ...
                  <div className="w-8 h-8 bg-gradient-to-br from-gray-700 to-gray-900 rounded-xl flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300">
                    ...
                    <SignedOut><UserCircle className="w-5 h-5 text-gray-300" /></SignedOut>
                  </div>
                </button>
            ...
            <div className="lg:hidden flex items-center gap-2">
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white"><Menu size={24} /></button>
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <MobileSidebar categories={categories} pathname={pathname} onClose={() => setIsMobileMenuOpen(false)} theme={theme} toggleTheme={toggleTheme} />
        )}
      </AnimatePresence>
    </>
  );
