...
export default function ServicesBanner() {
  return (
    <section className="relative bg-white/60 backdrop-blur-md py-16 sm:py-24">
      <div
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              key={index}
              variants={itemVariants}
              className="bg-white/80 rounded-xl shadow-lg p-8 text-center flex flex-col items-center transition-all duration-300 hover:shadow-2xl hover:-translate-y-2"
            >
              <div
        ...
        <motion.div
          variants={itemVariants}
          className="mt-16 sm:mt-20 pt-10 border-t border-gray-900/10"
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {stats.map((stat) => (
              <div key={stat.label} className="flex flex-col items-center">
                <div className="text-3xl sm:text-4xl font-bold text-rose-500">
                  {stat.value}
                </div>
