...
export default function ReviewSectionNew({ username, vendorName, onReviewUpdate }) {
  const { user } = useUser();
  ...
  const fetchReviews = useCallback(
    async (pageParam, isReset) => {
      ...
      try {
        ...
        const res = await fetch(`/api/vendor/profile/reviews?${query}`);
  ...
  const handleSubmitReview = async (data) => {
    ...
    try {
      ...
      const url = isEdit ? `/api/vendor/profile/reviews/${editingReview._id}` : `/api/vendor/profile/reviews`;

      const method = isEdit ? "PUT" : "POST";
      const payload = isEdit ? { ...data, username, action: "edit" } : { ...data, username };
  ...
  const handleVote = async (reviewId, type) => {
    ...
    try {
      await fetch(`/api/vendor/profile/reviews/${reviewId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, action: type }),
      });
    } catch (error) {
  ...
  const handleSubmitReply = async (reviewId, text) => {
    setSubmitting(true);
    try {
      const res = await fetch(`/api/vendor/profile/reviews/${reviewId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, action: "reply", text }),
      });
      const result = await res.json();
      if (result.success) {
  ...
  const handleDelete = async (reviewId) => {
    ...
    try {
      const res = await fetch(`/api/vendor/profile/reviews/${reviewId}?username=${username}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const handleFlag = async (reviewId) => {
    ...
    try {
      const res = await fetch(`/api/vendor/profile/reviews/${reviewId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, action: "flag", reason }),
      });
