...
const PostDetailModal = ({
  ...
  useEffect(() => {
    const loadInteractions = async () => {
      ...
      try {
        const response = await fetch(`/api/vendor/${vendorId}/profile/posts/interactions?${params}`);
        if (response.ok) {
  ...
  const handleDoubleTapLike = useCallback(async () => {
    ...
    if (!isLiked && currentPost?._id && user?.id) {
      ...
      try {
        const response = await fetch(`/api/vendor/${vendorId}/profile/posts/interactions`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            postId: currentPost._id,
  ...
  }, [isInteracting, currentPost?._id, isLiked, likes, user?.id, vendorId]);
  ...
  }, [isInteracting, currentPost?._id, isSaved, user?.id, vendorId]);
    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-900 px-5 pt-3 pb-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Plan Your Event</h3>
              <p className="text-sm text-gray-500 mt-1">Choose your event type to get started</p>
            </div>
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={resetAndClose}
        ...
        <div className="overflow-y-auto max-h-[calc(85vh-120px)] p-5">
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-blue-900 dark:text-blue-300 mb-1">Why choose {vendorName}?</p>
          ...
          {showQR ? (
            ...
            >
              ...
              <p className="text-center text-xs text-gray-500 dark:text-gray-400">
                Scan to visit {title ? title : "this Profile"}
              </p>
...
const VendorProfilePageWrapper = ({ initialReviews, initialProfile, initialVendor }) => {
  ...
  useEffect(() => {
    ...
    const totalPostLikes = initialProfile.posts?.reduce((total, post) => total + (post.likes?.length || 0), 0) || 0;

    const totalReelLikes = initialProfile.reels?.reduce((total, reel) => total + (reel.likes?.length || 0), 0) || 0;
              ...
              <>
                <div className="grid grid-cols-2 gap-[12px] mx-[15px]">
                  {posts.slice(0, 4).map((post, index) => {
                    ...
                    // Title mapping for each post position
                    const postTitles = [
                      "Work Details",
                      "Services Breakdown",
                      "Pricing & Packages",
                    ...
                    // Icon mapping for each post (optional - adds visual appeal)
                    const postIcons = [
                      <Briefcase size={14} />,
                      <Layers size={14} />,
                      <Tag size={14} />,
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Inner Card - Original Post Design (Unchanged) */}
                        <motion.div
                          whileTap={{ scale: 0.97 }}
                          onClick={() => {
                            if (playingVideoId) {
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 dark:text-slate-400">
                            {vendor?.imagesNew?.length} photos available
                          </p>
                        </div>
                      </div>
              ...
              >
                {profileLoading ? (
                  ...
                  <>
                    <motion.div
                      initial={false}
                      animate={{ height: isBioExpanded ? "auto" : "4rem" }}
                      transition={{
                        duration: 0.5,
                        ease: smoothEase,
                      }}
