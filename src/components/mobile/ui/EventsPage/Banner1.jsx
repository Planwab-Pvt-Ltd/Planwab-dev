  ...
  return (
    <section className="w-full">
      ...
      <div id="successful_booking_stats_section">
        <div className="relative flex items-center justify-center bg-[linear-gradient(143deg,#9E7C3D_21.68%,#D0AC6C_78.32%)] dark:bg-[linear-gradient(143deg,#422006_21.68%,#664d27_78.32%)] px-4 py-4 text-center text-sm font-semibold text-white dark:text-amber-100">
      ...
      {/* Button Container */}
      <div className="bottom-0 pb-12 px-6 flex flex-col items-center gap-4 z-10">
        <Link
          href={`/m/plan-my-event/${categoryKey}`}
          className={`w-full max-w-sm py-4 rounded-xl text-center text-base shadow-xl active:scale-95 transition-transform ${currentCategory.buttonTheme}`}
        >
          Book Now
        </Link>
        <Link
          href={`/m/vendors/marketplace/${categoryKey}`}
          className={`w-full max-w-sm py-4 rounded-xl text-center text-base shadow-xl active:scale-95 transition-transform bg-white/20 backdrop-blur-md text-gray-900 dark:text-white border border-white/40`}
        >
          Explore Vendors
        </Link>
      </div>
