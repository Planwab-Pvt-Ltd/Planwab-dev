...
const Toast = memo(({ message, type = "success", isVisible, onClose }) => {
  ...
  return (
    <motion.div
      ...
      className={`fixed top-4 left-4 right-4 md:left-auto md:right-4 md:w-96 z-[300] ${bg} text-white px-4 py-3 rounded-2xl shadow-xl flex items-center gap-3`}
    >
      <Info size={20} />
      ...
      {CHECKOUT_STEPS.map((step, index) => {
        ...
        return (
          <div key={step.id} className="flex flex-col items-center relative z-10">
            ...
            <span className={`text-[10px] md:text-xs font-semibold mt-1 ${active ? "text-gray-900" : "text-gray-400"}`}>
              {step.label}
            </span>
            {index < CHECKOUT_STEPS.length - 1 && (
              <div className="absolute left-1/2 top-5 md:top-6 w-full h-0.5 bg-gray-200 -z-10 ml-5">
                <motion.div
                  initial={{ width: "0%" }}
  ...
  return (
    ...
    >
      <div className="p-4 flex gap-3">
        <img src={item.image} alt={item.name} className="w-20 h-20 md:w-24 md:h-24 rounded-xl object-cover bg-gray-100" />
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-50">
            ...
            {item.addons?.length > 0 && (
              <button
                ...
                className="text-xs font-semibold text-gray-500 flex items-center gap-1 hover:text-gray-700 transition-colors"
              >
      ...
      {appliedCoupon ? (
        <div className="p-4 flex justify-between items-center">
          <div className="flex gap-3 items-center">
            ...
            <div>
              <p className="font-bold text-green-600 text-sm md:text-base">{appliedCoupon.code}</p>
              <p className="text-xs text-gray-500">{appliedCoupon.description}</p>
            </div>
          </div>
          <button onClick={onRemoveCoupon} className="hover:bg-gray-100 p-2 rounded-lg transition-colors">
            <X size={18} className="text-gray-400" />
          </button>
        </div>
      ) : (
        <>
          <button onClick={() => setExpanded(!expanded)} className="w-full p-4 flex justify-between items-center hover:bg-gray-50 transition-colors">
            <div className="flex gap-3 items-center">
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-100">
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 md:py-3 rounded-xl border border-gray-200 text-sm md:text-base"
                />
                <button
                  onClick={handleApply}
                  className="px-4 md:px-6 py-2 md:py-3 rounded-xl text-white font-bold text-sm md:text-base hover:opacity-90 transition-opacity"
              ...
              <div className="mt-3 space-y-2">
                {MOCK_COUPONS.map((c) => (
                  <button
                    ...
                    className="w-full text-left p-3 bg-white rounded-lg border border-gray-100 text-xs md:text-sm hover:border-gray-300 transition-colors"
                  >
...
const PriceSummary = memo(({ subtotal, discount, couponDiscount, taxes, total, colorPrimary, sticky = false }) => (
  <div className={`bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-6 ${sticky ? "lg:sticky lg:top-24" : ""}`}>
    <h3 className="font-bold text-gray-900 mb-3 md:text-lg">Price Details</h3>
    <div className="space-y-2 md:space-y-3 text-sm md:text-base">
      ...
      <div className="flex justify-between pt-3 border-t font-bold text-lg md:text-xl">
...
const FormInput = memo(({ label, error, ...props }) => (
  <div className="space-y-1.5">
    <label className="text-xs md:text-sm font-semibold text-gray-600">{label}</label>
    <div
    ...
    >
      ...
      <input
        {...props}
        className="flex-1 bg-transparent text-sm md:text-base font-medium outline-none text-gray-900 placeholder-gray-400"
      />
    </div>
    {error && (
      ...
      {cartItems.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="p-4 bg-gray-50 rounded-2xl flex flex-wrap gap-4 text-xs md:text-sm font-medium text-gray-600">
          ...
          <div className="hidden lg:block">
            <PriceSummary {...priceDetails} colorPrimary={colorPrimary} sticky />
          </div>
        </div>
      )}
    </div>
  ...
  <div className="px-4 md:px-6 lg:px-8 py-4 md:py-6">
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-6">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-6">
        <h3 className="font-bold text-lg md:text-xl mb-4 flex gap-2 items-center">
          <Calendar size={20} style={{ color: colorPrimary }} /> Event Details
        </h3>
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs md:text-sm font-semibold text-gray-600">Special Request</label>
            <textarea
              rows={3}
              className="w-full bg-gray-50 rounded-xl p-3 md:p-4 text-sm md:text-base border-transparent focus:bg-white focus:border-blue-400 border-2 outline-none"
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-6">
        <h3 className="font-bold text-lg md:text-xl mb-4 flex gap-2 items-center">
          <User size={20} style={{ color: colorPrimary }} /> Contact Info
        </h3>
        <div className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <FormInput
  ...
  <div className="px-4 md:px-6 lg:px-8 py-4 md:py-6">
    <div className="max-w-3xl mx-auto space-y-4 md:space-y-6">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 md:p-8 text-white">
        <p className="text-sm md:text-base opacity-80">Total Payable</p>
        <p className="text-3xl md:text-4xl font-black">₹{formatPrice(priceDetails.total)}</p>
        <div className="flex items-center gap-2 mt-2 text-xs md:text-sm opacity-80">
          <Shield size={14} /> Secure Payment Gateway
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-6">
        <h3 className="font-bold text-lg md:text-xl mb-4 flex gap-2 items-center">
          <CreditCard size={20} style={{ color: colorPrimary }} /> Payment Method
        </h3>
        <div className="space-y-3">
          {PAYMENT_METHODS.map((method) => (
            <button
              ...
              className={`w-full flex items-center gap-4 p-4 md:p-5 rounded-xl border-2 transition-all text-left ${
                paymentMethod === method.id ? "border-blue-500 bg-blue-50" : "border-gray-100 bg-white hover:border-gray-200"
            ...
            >
              ...
              <div className="flex-1">
                <p className="font-bold text-sm md:text-base text-gray-900">{method.label}</p>
              ...
              >
                {paymentMethod === method.id && <div className="w-2 h-2 md:w-2.5 md:h-2.5 bg-white rounded-full" />}
              </div>
            </button>
          ))}
        </div>
      ...
      {paymentMethod === "ONLINE" && (
        <div className="p-4 md:p-5 bg-blue-50 rounded-2xl flex gap-3 items-start">
          <Smartphone size={20} className="text-blue-600 mt-0.5" />
          <div>
            <p className="text-sm md:text-base font-bold text-blue-800">Razorpay Secure</p>
...
const ConfirmationStep = ({ cartItems, eventDetails, contactDetails, priceDetails, paymentMethod, colorPrimary }) => (
  <div className="px-4 md:px-6 lg:px-8 py-4 md:py-6">
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-6">
      <div className="bg-amber-50 p-4 md:p-5 rounded-xl flex gap-3 items-start">
        <AlertCircle size={20} className="text-amber-500 mt-0.5" />
        <div>
          <p className="font-bold text-amber-800 text-sm md:text-base">Review your order</p>
          <p className="text-xs md:text-sm text-amber-700">Verify all details before confirming.</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 md:p-5 bg-gray-50 border-b border-gray-100 font-bold text-base md:text-lg flex gap-2 items-center">
          <Receipt size={20} style={{ color: colorPrimary }} /> Order Summary
        </div>
        <div className="p-4 md:p-6 space-y-4 md:space-y-5">
          <div className="p-3 md:p-4 bg-gray-50 rounded-xl space-y-1">
            <p className="text-xs font-bold text-gray-400 uppercase">Event</p>
            <p className="font-semibold text-sm md:text-base">{eventDetails.eventName}</p>
          ...
          <div>
            ...
            {cartItems.map((item, i) => (
              <div key={i} className="flex justify-between text-sm md:text-base py-2 border-b border-gray-50 last:border-0">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-5 flex justify-between items-center">
        <div>
          <p className="text-xs md:text-sm text-gray-500">Total Payable</p>
          <p className="text-xl md:text-2xl font-black" style={{ color: colorPrimary }}>
            ₹{formatPrice(priceDetails.total)}
          </p>
        </div>
        <div className="px-3 py-1.5 md:px-4 md:py-2 bg-blue-50 text-blue-600 rounded-lg text-xs md:text-sm font-bold">
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="w-24 h-24 md:w-32 md:h-32 bg-green-100 rounded-full flex items-center justify-center mb-6"
        >
          <CheckCircle size={48} className="text-green-600 md:w-16 md:h-16" />
        </motion.div>
        <h1 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">Booking Confirmed!</h1>
        <p className="text-gray-500 mb-8 text-center text-sm md:text-base max-w-md">
          Your order has been placed successfully.
          <br />
          Vendors will contact you shortly.
        </p>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 w-full max-w-md mb-6">
          <div className="flex justify-between items-center mb-3">
            <span className="text-xs md:text-sm font-bold text-gray-400 uppercase">Order ID</span>
            <span className="font-mono font-bold text-sm md:text-base">{finalOrderData.orderId}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-xs md:text-sm font-bold text-gray-400 uppercase">Amount Paid</span>
            <span className="font-bold text-green-600 text-base md:text-lg">₹{formatPrice(finalOrderData.amount)}</span>
          </div>
        </div>

        <div className="w-full max-w-md space-y-3">
          <Link href="/user/bookings">
            <button className="w-full py-4 rounded-xl bg-blue-600 text-white font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors">
              View My Bookings
            </button>
          </Link>

          <Link href="/vendors/marketplace">
            <button className="w-full py-4 rounded-xl text-gray-500 font-bold hover:bg-gray-100 transition-colors">Continue Shopping</button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 md:pb-28">
      ...
      <div className="bg-white sticky top-0 z-20 border-b border-gray-100 safe-area-top">
        <div className="max-w-7xl mx-auto flex items-center p-4 md:px-6 lg:px-8">
          <button
          ...
          <span className="font-bold text-lg md:text-xl ml-2">Checkout</span>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 md:px-6 lg:px-8 z-20 safe-area-bottom">
        <div className="max-w-3xl mx-auto">
          <button
            onClick={handleNext}
            disabled={isProcessing}
            className="w-full py-4 md:py-5 rounded-xl text-white font-bold text-base md:text-lg shadow-lg flex items-center justify-center gap-2 disabled:opacity-70 hover:opacity-90 transition-opacity"
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-white/90 z-50 flex flex-col items-center justify-center">
          <div className="w-16 h-16 md:w-20 md:h-20 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin mb-4" />
          <p className="font-bold text-xl md:text-2xl">Processing Payment...</p>
          <p className="text-sm md:text-base text-gray-500">Please do not close this window.</p>
        </div>
      )}
    </div>
  );
