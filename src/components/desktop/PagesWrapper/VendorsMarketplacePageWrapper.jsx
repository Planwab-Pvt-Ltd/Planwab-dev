...
import {
  MapPin, Star, Users, Palette, UserCheck, Heart, Camera, Brush,
  ChevronDown, LayoutGrid, Square, SlidersHorizontal, X, Check, Filter, Search,
  Sparkles, TrendingUp, Clock, Shield, Award, ChevronLeft, ChevronRight,
  Eye, Share2, Phone, Mail, Calendar, DollarSign, Zap,
} from "lucide-react";
                ...
                {pageNumbers.map((num, index) => (
                    ...
                    {num === '...' ? (
                        ...
                        <button
                            ...
                            className={`w-10 h-10 rounded-lg transition-colors ${currentPage === num ? 'bg-amber-500 text-white font-bold' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                        >
                            {num}
                        </button>
                    )}
            ...
            <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <span className="text-xs">
                  {vendor?.bookings || 0} bookings in last 3 months
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Star
          ...
          {vendorCategories.map((cat) => (
            ...
            >
              <input
                ...
                className={`w-5 h-5 rounded text-${color}-600 focus:ring-${color}-500 border-gray-300 dark:border-gray-500 dark:bg-gray-600 mr-3 transition-colors ${selectedCategories.includes(cat) ? 'bg-blue-600 border-blue-600' : ''}`}
              />
              <span className={`capitalize ${selectedCategories.includes(cat) ? "text-gray-900 dark:text-gray-100 font-medium" : "text-gray-600 dark:text-gray-300"}`}>
                {cat}
              </span>
            </label>
          ))}
            ...
            {availableCities.map((city) => (
              ...
              >
                <input
                  ...
                  className={`w-5 h-5 rounded text-${color}-600 focus:ring-${color}-500 border-gray-300 dark:border-gray-500 dark:bg-gray-600 mr-3 transition-colors ${selectedLocations.includes(city) ? 'bg-blue-600 border-blue-600' : ''}`}
                />
                <span className={`${selectedLocations.includes(city) ? "text-gray-900 dark:text-gray-100 font-medium" : "text-gray-600 dark:text-gray-300"}`}>
                  {city}
                </span>
              </label>
            ))}
...
export default function MarketplacePageWrapper() {
    ...
    }, [currentPage, sortBy, debouncedPriceRange, debouncedSearchQuery, selectedCategories, showFeaturedOnly, selectedLocations, debouncedGuestCapacity]);
